function parseDiscourseVerification(message, discordId) {
...
function parseDiscourseCheck(message, discordId) {
  // This discordId comes directly from the author of the message (discord.js)
  // Therefore, only two things can happen: get a valid ID or get 'undefined'.
  if (typeof discordId === 'undefined') {
    throw new Error(
  ...
  if (splitMessage.length < 2) {
    throw new Error(
      'Parsing command failed, reason: not enough arguments. Expecting a minimum of two: the verification code and discourse username.',
    )
  }
...
module.exports = { parseDiscourseVerification, parseDiscourseCheck }
